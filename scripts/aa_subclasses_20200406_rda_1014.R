...
bd <- read.csv2(paste0(datadir,"bd_simulation_28_2010_2014_20200403.csv"))
df0 <- bd
...
for(province in unique(ar$NOM)){
  ...
  # loss0010 <- as.data.frame.matrix(subc/sum(subc)*s[3,"strRS_area_estimate"])        # Comme on a d?j? restreint la BD, il n'y a que 3 lignes
  # ci0010   <- as.data.frame.matrix(subc/sum(subc)*s[3,"strRS_confidence_interval"])  # Comme on a d?j? restreint la BD, il n'y a que 3 lignes
  
  prop1014 <- as.data.frame.matrix(subc/sum(subc))
  loss1014 <- as.data.frame.matrix(subc/sum(subc)*s[3,"strRS_area_estimate"])       # Comme on a d?j? restreint la BD, il n'y a que 3 lignes
  ci1014   <- as.data.frame.matrix(subc/sum(subc)*s[3,"strRS_confidence_interval"]) # Comme on a d?j? restreint la BD, il n'y a que 3 lignes
  ...
  #write.csv(prov0010,paste0(drvdir,province,"_0010_r.csv"),row.names = F) # pour R?mi en CSV normal
