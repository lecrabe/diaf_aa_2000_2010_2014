...
bd <- read.csv2(paste0(datadir,"Bd_14_16_27092019_chg_map_cd_rdc26_cd.csv"))

##### PRENDRE TOUS LES POINTS DES TRANSITIONS STABLES
#bd$ce_prov_transition <- paste0(bd$map_province,bd$ce_transition)

#bd$map_cl <- paste0(bd$map_province,bd$map_class1416)
...
ar <- read.csv(paste0(datadir,"comptage_pixel_map_14_16_inputs_sepal_15102019.csv"))
...
legend  <- levels(as.factor(ar[,ar_code]))
tmpa <- tapply(ar[,ar_area],ar[,ar_code],sum)
#legend <- levels(as.factor(df0[, ref_code]))
areas   <- data.frame(cbind(legend,tmpa))
...
for(simulation in 1:1){
  ...
  for(class in unique(df0[,map_code])){
    tmp <- sample(df0[df0[,map_code] == class,"unique_id"],
                  min(a4[a4$prov_class == class,"adjusted"],nrow(df0[df0[,map_code] == class,])))
...
write.csv2(df,paste0(datadir,"bd_1416_sim_20201303.csv"),row.names= F)

names(resultat) <- c("area_NF","area_F","area_DEF","ci_NF","ci_F","ci_DEF")

resultat

write.csv2(resultat,paste0(datadir,"res_sim_2014_2016_2020.csv"),row.names = F)
