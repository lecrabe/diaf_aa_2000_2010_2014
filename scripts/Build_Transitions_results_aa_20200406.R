...
# Preciser le chemin du dossier vers le tableaux de donn?es
...
# Preciser le nom du fichier CSV de donn?es (on le charge dans l'objet "CONV")                                             
CONV <- bd
head(CONV)
...
# enregistrement (interm?ddiaire) du fichier "prop" sur lequel on calcul les proportion 
write.table(CONV_fin, paste(datadir,"CONV_fin_R.csv", sep=""), dec=".", sep=";", row.names=F)

####
####  Build summary statistics for JP
####

# On g?n?re un SHP pour tir? les provinces
library(raster)
library(rgdal)

# on lie le fichier des provinces
PROVINCES <- shapefile(paste0(datadir,"RDC_Province_26.shp"))
