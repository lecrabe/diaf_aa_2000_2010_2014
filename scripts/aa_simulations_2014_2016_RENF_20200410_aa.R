...
table(bd$map_cl,bd$prov_transition)
all(bd$map_cl==bd$prov_transition)
all(bd$ref_cl==bd$ce_prov_transition)

  
ar <- read.csv2(paste0(datadir,"comptage_pixel_map_14_16_inputs_sepal_15102019_calcul_emission.csv"))
ar$cls_cl <- paste0(ar$province,ar$class1416)


table(ar$transition_clean,ar$cls_cl)

##### SELECTIONNER SEULEMENT LES SINGLETS
#df0 <- bd[bd$count ==1 & bd$periode == '2014_2016',]


# verifier les id uniques
length(unique(bd$id))
table(df0$map_cl)

# verifier les location x y uniques
bd$location_id <- paste0(bd$location_x,bd$location_y)
length(unique(bd$location_id))

# creation d'un nouvel ID unique
bd$unique_id <- row(bd)[,1]
length(unique(bd$unique_id))
...
temp   <- data.frame(cbind(tapply(ar[,ar_area],ar[,ar_code],sum)))
areas <- cbind(row.names(temp),temp)
...
resultat <- data.frame(matrix(ncol=7,nrow=0))
names(resultat) <- c("numero_sim","area_NF","area_F","area_DEF","ci_NF","ci_F","ci_DEF")
...
for(simulation in 1:30){
  ...
  resultat <- rbind(resultat,c(simulation,r1,i1))

  write.csv2(s1,paste0(datadir,"res_sim",simulation,"_2014_2016.csv"),row.names = F)  
  
  write.csv2(df,paste0(datadir,"bd_simulation_",simulation,"_2014_2016_20200403.csv"),row.names= F)
...
names(resultat) <- c("num_sim","area_NF","area_F","area_DEF","ci_NF","ci_F","ci_DEF")
arrange(resultat,area_DEF)

write.csv2(resultat,paste0(datadir,"res_sim_2014_2016_20200414.csv"),row.names = F)
